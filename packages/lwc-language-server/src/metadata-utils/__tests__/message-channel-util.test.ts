...
`;
    await validate(indexMessageChannels, 'sfdx-workspace', 'force-app', 'messageservice.d.ts', expectedDTS);

    // Check that messagechannels.d.ts gets generated properly for the 2 existing message channels
    const expectedDTS2 = `declare module "@salesforce/messageChannel/Channel1__c" {
    var Channel1: string;
    export default Channel1;
}
declare module "@salesforce/messageChannel/Channel2__c" {
    var Channel2: string;
    export default Channel2;
}
`;
    await validate(indexMessageChannels, 'sfdx-workspace', 'force-app', 'messagechannels.d.ts', expectedDTS2);
    done();
});
