...
it('returns a list of all the template compilation errors', () => {
    ...
    expect(diagnostics[0].range).toMatchObject({ start: { character: 20 }, end: { character: 47 } });
    expect(diagnostics[1].message).toMatch(/Invalid expression/);
    expect(diagnostics[1].range).toMatchObject({ start: { character: 48 }, end: { character: 63 } });
});