...
const htmlScriptContents: { [type: string]: boolean } = {
...
export function createScanner(
    ...
    function internalScan(): TokenType {
        ...
        switch (state) {
            ...
            case ScannerState.WithinTag:
                ...
                if (stream.advanceIfChar(_RAN)) {
                    ...
                    if (lastTag === 'script') {
                        if (
                            lastTypeValue &&
                            htmlScriptContents[lastTypeValue]
                        ) {
                            // stay in html
                            state = ScannerState.WithinContent;
                        } else {
            ...
            case ScannerState.WithinScriptContent:
                ...
                let sciptState = 1;
                while (!stream.eos()) {
                    let match = stream.advanceIfRegExp(
                        /<!--|-->|<\/?script\s*\/?>?/i,
                    );
                    if (match.length === 0) {
                        stream.goToEnd();
    ...
    return {
        ...
        getTokenText: () =>
            stream.getSource().substring(tokenOffset, stream.pos()),
