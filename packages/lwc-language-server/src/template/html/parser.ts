...
import { TokenType, createScanner } from './scanner';
import { findFirst } from './utils';
import { isEmptyElement } from './html-tags';
...
export function parse(text: string): HTMLDocument {
    ...
    let htmlDocument = new Node(0, text.length, [], {} as Node);
    let curr = htmlDocument;
    let endTagStart: number = -1;
    let pendingAttribute: any;
    let attributes: any;
