...
import { pathExists, ensureDir, ensureDirSync } from './fs-utils';
...
const findCoreESLint = async (): Promise<string> => {
    const eslintToolDir = path.join(homedir(), 'tools', 'eslint-tool');
    if (!(await pathExists(eslintToolDir))) {
