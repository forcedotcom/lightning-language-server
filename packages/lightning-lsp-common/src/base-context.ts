...
import ejs from 'ejs';
import { WorkspaceTypes, WorkspaceType, detectWorkspaceType, getSfdxProjectFile } from './shared';
...
export abstract class BaseWorkspaceContext {
    ...
    public constructor(workspaceRoots: string[] | string) {
        ...
        if (this.type === WorkspaceTypes.SFDX) {
    ...
    private async writeCoreJsconfig(): Promise<void> {
        ...
        for (const modulesDir of modulesDirs) {
            ...
            try {
                ...
                const coreDir = this.type === WorkspaceTypes.CORE_ALL ? this.workspaceRoots[0] : path.dirname(this.workspaceRoots[0]);
